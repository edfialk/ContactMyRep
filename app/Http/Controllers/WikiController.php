...
class WikiController extends Controller
{
    ...
    const SENATE_URL = 'https://en.wikipedia.org/wiki/List_of_current_United_States_Senators';
    const HOUSE_URL = 'https://en.wikipedia.org/wiki/Current_members_of_the_United_States_House_of_Representatives';
    const GOVERNOR_URL = 'https://en.wikipedia.org/wiki/List_of_current_United_States_governors';
    ...
    {
        $x = $this->xpath(self::GOVERNOR_URL);
        $table = $x->query('//table[contains(@class, "sortable")]')[0];
        $rows = $x->query('.//tr[td]', $table);
        foreach ($rows as $row) {
